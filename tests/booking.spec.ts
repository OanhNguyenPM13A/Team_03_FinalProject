...
test.describe.serial('Module 3: Booking ====>', () => {

    // TC: Verify booking form displays correctly with all elements
    test('TC17: Verify booking form displays all elements', async ({ page }) => {
    ...
    test('TC18: Validate price breakdown calculations', async ({ page }) => {
        // Initialize page objects
        const homePage = new HomePage(page);
        const booking = new Booking(page);
    ...
    test('TC19: Booking successful (end-to-end flow)', async ({ page }) => {
        // Initialize page objects
        const homePage = new HomePage(page);
    ...
    test('TC20: Booking failed - Not logged in', async ({ page }) => {
        // Initialize page objects
        const homePage = new HomePage(page);
        const booking = new Booking(page);
    ...
    // TC20: Verify booking history displays correctly
    test('TC: Verify booking history displays correctly', async ({ page }) => {
        // Initialize page objects
        const homePage = new HomePage(page);
        const header = new HeaderComponent(page);
        const loginPopup = new LoginPopup(page);
        ...
        // Step7: Navigate to profile page
        await profilePage.goto();

        // Step8: Verify booking history section is visible
        await expect(profilePage.bookingHeading).toBeVisible();

        const bookingList = profilePage.page.locator('div.space-y-6');
        await expect(bookingList).toBeVisible();

        const bookedRoomLinks = bookingList.locator('a[href^="/room-detail/"]');
        // Step9: Verify at least one room card is displayed
        await expect(bookedRoomLinks.first()).toBeVisible();

        // Step10: Verify the room card UI exists inside link
        const firstCard = bookedRoomLinks.first().locator('.ant-card');
        await expect(firstCard).toBeVisible();

        // Step11: Verify price "/ đêm" appears somewhere in card
        await expect(firstCard).toContainText('/ đêm');

        // Step12: Verify clicking card goes to room detail
        await bookedRoomLinks.first().click();
        await expect(profilePage.page).toHaveURL(/\/room-detail\/\d+$/);

    });

    // TC: Booking failed - Invalid date
    test('TC21: Booking failed - Invalid date', async ({ page }) => {
        // Initialize page objects
        const homePage = new HomePage(page);
        const header = new HeaderComponent(page);
        const loginPopup = new LoginPopup(page);
